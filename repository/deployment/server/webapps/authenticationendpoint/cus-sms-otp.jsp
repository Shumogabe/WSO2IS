...
<layout:main layoutName="<%= layout %>" layoutFileRelativePath="<%= layoutFileRelativePath %>" data="<%= layoutData %>">
    ...
    <layout:component componentName="MainSection">
        <div class="ui segment">
                <%-- page content --%>
            <p style="font-size: 200%">Xác thực SMS OTP</p>
            <p id="CUSTOM_SMS_AUTHENTICATOR_RESPONSE" style="font-size: 110%">Mã xác nhận đã được gửi qua SĐT của bạn <b><%=encodeMobileNumber((String) FrameworkUtils.getContextData(request).getProperty("CUSTOM_SMS_AUTHENTICATOR_RESPONSE"))%></b></p>
            <div class="ui negative message" id="failed-msg"></div>
            <div class="ui divider hidden"></div>
            <div id="alertDiv"></div>
            <div class="segment-form">
                <form class="ui large form" id="custom_sms_otp_form" action="<%=commonauthURL%>" method="POST">
                    <input type="text" name="custom_sms_otp" id="custom_sms_otp" hidden>
                    <input type='hidden' name='custom_resend_sms_otp' value='false' id="custom_resend_sms_otp"/>
                    ...
                    <div style="display: flex; flex-direction: column; margin-top: 15px; justify-items: center">
                        ...
                        <p style="margin: auto; padding: 15px ; display: none" id="resendOtp">Bạn chưa nhận được mã?<a style="cursor: pointer" onclick="resendCode()"> Gửi lại OTP</a></p>
                    </div>
...
<script>
    ...
    function resendCode() {
        $("#custom_resend_sms_otp").val("true");
        document.getElementById("custom_sms_otp_form").submit();
    }

    function submitForm() {
        $("#custom_sms_otp").val(getOtp());
        document.getElementById("custom_sms_otp_form").submit();
    }
    ...
    const SMS = '<%=FrameworkUtils.getContextData(request).getProperty("CUSTOM_SMS_AUTHENTICATOR_RESPONSE")%>';
    ...
    } else if (SMS === "null" || SMS.trim()==="") {
        showMessage("Bạn cần thêm phương thức bằng SMS!");
        document.getElementById("CUSTOM_SMS_AUTHENTICATOR_RESPONSE").style.display="none";
        document.getElementById("custom_sms_otp_form").style.display="none";
