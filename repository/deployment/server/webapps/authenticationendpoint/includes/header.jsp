...
<title><%=getCustomValueProperty(request, "page-title")%></title>

<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%
    if (getCustomValueProperty(request, "header-enable").equals("true")) {
%>
<div style="position: fixed; top: 0; width: 100%; z-index: 1">
    <div style="
            display: flex;
            justify-content: start;
            background-color: <%=getCustomValueProperty(request,"header-background-color")%>;
            color: <%=getCustomValueProperty(request,"header-color")%>;
            padding: 12px;
            ">
        <div>
            <img style="margin: 0 20px; max-height: 20px ;overflow: hidden" src="<%=getCustomValueProperty(request, "header-logo")%>" alt="<%=getCustomValueProperty(request, "header-logo-alt")%>"/>
        </div>
        <div><%=getCustomValueProperty(request, "header-title")%>
        </div>
    </div>
</div>
<%
...
%>
<script type="text/javascript" src="libs/jquery_3.6.0/jquery-3.6.0.min.js"></script>
