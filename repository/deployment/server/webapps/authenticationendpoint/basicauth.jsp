...
<form class="ui large form" action="<%=loginFormActionURL%>" method="post" id="loginForm">
    ...
    <label style="display: flex; justify-content: end;">
        <a class="forgot-password" style="margin: -10px 0; font-style: italic" href="<%=getCustomValueProperty(request,"forgot-password-url")%>">Lấy lại mật khẩu ?</a>
    </label>
    <div class="mt-0">
...
<form id="ldap_login_form" action="../commonauth" method="POST" style="display: none">
    <input type="text" name="usernameLdap" id="usernameLdap"/>
    <input type="password" name="passwordLdap" id="passwordLdap"/>
    <input value="<%=request.getParameter("sessionDataKey")%>" name="sessionDataKey"/>
</form>

<button type="button" class="btn btn-danger" data-dismiss="modal" id="btn-submit-error" style="display: none">Xác nhận</button>


<jsp:directive.include file="model.jsp"/>


<script>

    function handleUsernameAuthenticator(username, password) {
        document.getElementById("username").value = username;
        document.getElementById("password").value = password;
        localStorage.setItem("type","face_fingerprint");
        submitForm();
    }
    ...
    function showDiv(type) {
        if(type === "btnFaceId") {
            document.getElementById("btnFaceId").style.display = "block";
        }
        else if(type === "btnFingerprint") {
            document.getElementById("btnFingerprint").style.display = "block";
        }
    }
    ...
    function hideDiv(type) {
        if(type === "btnFaceId") {
            ...
            document.getElementById("btnFingerprint").style.display = "none";
        }
    }
    ...
    function isLoginAD() {
        ...
        let hasLdapAuthenticator = <%=localAuthenticatorNames.contains("LDAP_AUTHENTICATOR")%>
        return !isNumber && hasLdapAuthenticator;
    }


    function loginAD() {
        document.getElementById("usernameLdap").value = document.getElementById("username").value;
        document.getElementById("passwordLdap").value = document.getElementById("password").value;
        document.getElementById("ldap_login_form").submit();
    }

    document.getElementById("btnLoginUsername").addEventListener("click", function(event) {
        if(isLoginAD()) {
            console.log("loginAD");
            loginAD();
        }
    ...
    function showMessage() {
        let CUSTOM_ERROR_CODE = '<%=FrameworkUtils.getContextData(request).getProperty("CUS_CODE")%>';
        if(CUSTOM_ERROR_CODE==="NOT_HAVE_USER") {
            document.getElementById("error-msg").innerText="Không tìm thấy người dùng! Vui lòng đăng ký";
        }
    }
    showMessage();
